...
% Edited by Shoutik Mukherjee -- 07/26/2018:Md usage modified to match with
% formatting
...
%             end
        ...
        % Edit - SM
            if rows(ee)==cols(ee), continue; end
            AGC(t,rows(ee),cols(ee)) = 1 - alpham - ncx2cdf( chi2inv(1-alpham,Md(t,rows(ee),cols(ee))),Md(t,rows(ee),cols(ee)), nuk(t,rows(ee),cols(ee)) );
            whati = wfk{rows(ee)};
            CellC = 1:Ncells; CellC(rows(ee))=[];
            jji = find(CellC == cols(ee));
            if sign(whati(t,(jji-1)*Mhc+Mhcs+2:jji*Mhc+Mhcs+1)*Whc') ~=0
